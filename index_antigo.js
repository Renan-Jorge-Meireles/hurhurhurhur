...
app.post('/funcionarios', (req, res) => {
    ...
    const query = `INSERT INTO funcionarios (nome, telefone, email, cpf, endereco) VALUES (?, ?, ?, ?, ?)`;
    db.run(query, [nome, telefone, email, cpf, endereco], function (err) {
        if (err) {
...
app.get('/funcionarios', (req, res) => {
    ...
    if (cpf) {
        ...
        const query = `SELECT * FROM funcionarios WHERE cpf LIKE ?`;
...
app.put('/funcionarios/cpf/:cpf', (req, res) => {
    ...
    const query = `UPDATE funcionarios SET nome = ?, telefone = ?, email = ?, endereco = ? WHERE cpf = ?`;
    db.run(query, [nome, telefone, email, endereco, cpf], function (err) {
        if (err) {
...
app.put('/produtos/:id', (req, res) => {
    ...
    const query = `UPDATE produtos SET nome = ?, preco = ?, descricao = ?, categoria = ?, quantidade_estoque = ?, fornecedor_id = ? WHERE id = ?`;
    db.run(query, [nome, preco, descricao, categoria, quantidade_estoque, fornecedor_id, id], function (err) {
        if (err) {
...
app.post('/fornecedores', (req, res) => {
    ...
    if (!cnpj || !nome) {
        return res.status(400).send('CNPJ e nome são obrigatórios.');
    }

    const query = `INSERT INTO fornecedor (cnpj, nome, telefone) VALUES (?, ?, ?)`;
    db.run(query, [cnpj, nome, telefone], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
                return res.status(400).send('Já existe um fornecedor cadastrado com este CNPJ!');
            }
...
app.get('/fornecedores', (req, res) => {
    ...
    if (cnpj) {
        ...
        const query = `SELECT * FROM fornecedor WHERE cnpj LIKE ?`;
...
app.put('/fornecedores/:id', (req, res) => {
    ...
    const query = `UPDATE fornecedor SET cnpj = ?, nome = ?, telefone = ? WHERE id_fornecedor = ?`;
    db.run(query, [cnpj, nome, telefone, id], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
                return res.status(400).send('Já existe outro fornecedor com este CNPJ!');
            }
